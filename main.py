...
def pipeline(image, cars, model, frame_num, heat_map='n'):
    ...
    model = joblib.load('pipe-0.97-1.pkl') # load trained linear SVC from a .pkl file
    cars = Cars()
    frame_num = 0
    data = pipeline(image, cars, model, 0, heat_map='y')
    ...
    if heat_map is 'y':
         ...
         return image, heat_map, image_with_heatmap, probs
...
if __name__ == "__main__":
    ...
    model = joblib.load('pipe-0.9583333333333334-1.pkl') # load trained linear SVC from a .pkl file
